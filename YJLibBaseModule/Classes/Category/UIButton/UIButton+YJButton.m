...
static char *WDIndicatorViewKey = "WDIndicatorViewKey";

/// 开始loading动画
- (void)yjStartAnimation {
    self.enabled = NO;
    
    UIView *backView = objc_getAssociatedObject(self, &WDIndicatorViewKey);
    ...
    if (!backView) {
        ...
        objc_setAssociatedObject(self, &WDIndicatorViewKey, backView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
...
- (void)yjStopAnimation {
    UIView *indicatorView = objc_getAssociatedObject(self, &WDIndicatorViewKey);
